...
<p align="center">
<a href="https://uk.mathworks.com/matlabcentral/fileexchange/183498-arcade"><img src="https://www.mathworks.com/matlabcentral/images/matlab-file-exchange.svg" width="144.5"></a>&nbsp;&nbsp;
<a href="https://matlab.mathworks.com/open/github/v1?repo=Vasileios-Bellos/Arcade"><img src="https://www.mathworks.com/images/responsive/global/open-in-matlab-online.svg" width="216"></a>&nbsp;&nbsp;
...
Wireframe polygons with random vertices that split on impact - large asteroids split into two medium, medium split into two small, each faster than the last. The spaceship follows your cursor and auto-fires at the nearest asteroid. Wave-based with increasing asteroid count and speed.
...
| **Flappy Bird** | Space / Up / Click to flap |
...
Scores persist in `data/scores.mat` (auto-created on first play). Each record stores high score, max combo, total plays, and cumulative session time.
...
packaging/
    scripts/             build scripts (toolbox, executable, icon)
...
No MATLAB license required. [`ArcadeInstaller.exe`](packaging/ArcadeInstaller.exe) bundles a web installer that downloads the free [MATLAB Runtime R2025b](https://www.mathworks.com/products/compiler/matlab-runtime.html) automatically. See the [packaging guide](packaging/) for details.
...
buildToolbox       % creates packaging/Arcade.mltbx
buildExecutable    % creates packaging/ArcadeInstaller.exe
generateIcon       % creates packaging/assets/icon.png, splash.png
```
